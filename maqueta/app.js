...
// Descripciones de los estilos segun manual IMCE 2015
const styleDescriptions = {
    E1: {
        name: "Dirigir",
        subtitle: "Alta Tarea - Baja Relacion",
        color: "#e74c3c",
        description: "El lider define los roles y tareas del colaborador y lo supervisa de cerca. Las decisiones las toma el lider y las comunica al colaborador. Comportamiento alto en tarea y bajo en relacionamiento. Involucra decirle claramente a las personas lo que deben hacer, como hacerlo, donde y cuando, y luego supervisar estrechamente su desempeno."
    },
    E2: {
        name: "Instruir",
        subtitle: "Alta Tarea - Alta Relacion",
        color: "#f39c12",
        description: "El lider define roles y tareas pero busca ideas y sugerencias del colaborador. Las decisiones las sigue tomando el lider, pero la comunicacion es bidireccional. Comportamiento alto en tarea y alto en relacionamiento. El lider sigue proporcionando direccion pero tambien explica las decisiones, pide sugerencias y reconoce los avances."
    },
    E3: {
        name: "Apoyar",
        subtitle: "Baja Tarea - Alta Relacion",
        color: "#27ae60",
        description: "El lider y el colaborador participan juntos en la toma de decisiones. El rol principal del lider es facilitar, comunicar y dar apoyo. Comportamiento bajo en tarea y alto en relacionamiento. Involucra escuchar a las personas, facilitar su compromiso en la resolucion de problemas y proporcionar estimulos para sus esfuerzos."
    },
    E4: {
        name: "Delegar",
        subtitle: "Baja Tarea - Baja Relacion",
        color: "#3498db",
        description: "El lider delega la responsabilidad de las decisiones cotidianas al colaborador, proporcionando minima supervision y apoyo. Comportamiento bajo en tarea y bajo en relacionamiento. Traspasa responsabilidades de las decisiones cotidianas al equipo, buscando el desarrollo de la autogestion."
    }
};
...
function renderLiderazgoResults(container) {
    ...
    container.innerHTML = `
        <div id="report-content" style="color:#000">
            <div class="report-candidate-section mb-3" style="border-radius:var(--pp-radius)">
            ...
            <h5 class="report-section-title" style="color:#000">1. Puntajes por Estilo de Liderazgo</h5>
            <div class="row g-3 mb-4">
                ${['E1','E2','E3','E4'].map(style => {
                    const s = styleDescriptions[style]; const score = results.styleScores[style];
                    return `<div class="col-6 col-md-3"><div class="score-box" style="--score-color:${s.color}"><div class="score-label" style="color:#000">${style} - ${s.name}</div><div class="score-value">${score}</div><div class="progress" style="height:8px"><div class="progress-bar" style="background:${s.color};width:${(score/12)*100}%"></div></div><small style="color:#000">${s.subtitle}</small></div></div>`;
                }).join('')}
            </div>

            <h5 class="report-section-title" style="color:#000">2. Grafico Comparativo</h5>
            <div class="chart-container mb-4"><canvas id="chart-styles" width="500" height="280"></canvas></div>

            <h5 class="report-section-title" style="color:#000">3. Estilo Predominante</h5>
            <div class="d-flex gap-4 align-items-start mb-3">
                ...
                <div style="color:#000">
                    <h5 class="mb-1" style="color:#000">${results.predominantStyle} - ${pStyle.name}</h5>
                    <p style="color:#000" class="mb-2">${pStyle.subtitle}</p>
                    <p style="color:#000">${pStyle.description}</p>
            ...
            <h5 class="report-section-title" style="color:#000">4. Descripcion de los 4 Estilos</h5>
            <div class="row g-3 mb-4">
                ${['E1','E2','E3','E4'].map(style => {
                    const s = styleDescriptions[style];
                    const isPredom = style === results.predominantStyle;
                    return `<div class="col-md-6"><div class="p-3 rounded" style="border:1px solid ${isPredom ? s.color : '#e2e8f0'};border-top:4px solid ${s.color};background:${isPredom ? s.color+'08' : '#fff'}">
                        <h6 style="color:#000">${style} - ${s.name} ${isPredom ? '<span class="badge bg-primary">Predominante</span>' : ''}</h6>
                        <small style="color:#000">${s.subtitle}</small>
                        <p class="small mt-2 mb-0" style="color:#000">${s.description}</p>
                    </div></div>`;
                }).join('')}
            </div>

            <h5 class="report-section-title" style="color:#000">5. Detalle de Respuestas por Situacion</h5>
            <table class="table table-striped table-bordered">
                ...
                <tbody>${results.detailPerSituation.map(d => {
                    ...
                    return `<tr style="color:#000"><td>Situacion ${d.situation}</td><td><strong>Alt. ${d.answer}</strong></td><td><span class="style-tag" style="background:${s.color}">${d.style} - ${s.name}</span></td></tr>`;
                }).join('')}</tbody>
            </table>

            <div class="p-3 rounded mt-3" style="background:#f8fafc;border:1px solid #e2e8f0;color:#000">
                ...
                <p class="small fst-italic mb-0">Este reporte es un instrumento de autodiagnostico. No constituye una evaluacion clinica.</p>
            </div>
        </div>
        <div class="modal-test-footer">
            <button class="btn btn-primary" onclick="downloadPDF()"><i class="fas fa-file-pdf"></i> Descargar PDF</button>
...
function calculateResults() {
    ...
    return { styleScores, predominantStyle, detailPerSituation };
}
